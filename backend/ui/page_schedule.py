...
from backend.core.widgets import (ScrollFrame, TabBar, page_title, card,
                     kv_row, action_btn, status_lbl, section_divider)
from backend.core.schedule_event import (ScheduleEvent, DAYS, CATEGORIES,
                             CATEGORY_COLORS, fmt_time, fmt_duration)
from backend.core.time_engine import (get_free_blocks, largest_free_block,
                          weekly_availability, detect_conflicts,
                          weekly_income_summary, opportunity_cost)
import backend.core.week_engine as week_engine
from backend.core.config import DAY_START, DAY_END
...
class SchedulePage(tk.Frame):
    ...
    def _my_events(self):
        ...
        def refresh(*_):
            ...
            for ev in matches:
                ...
                hrs = self._shift_hours(ev)
                from backend.core.schedule_event import fmt_time, fmt_duration
                time_str = (f"{fmt_time(ev.start_time)} – {fmt_time(ev.end_time)}"
                            f"  ({fmt_duration(hrs)})"
                            + ("  +1 day" if ev.end_time < ev.start_time else ""))
                tk.Label(info, text=time_str,
    ...
    def _get_known_rate(self, job_name: str) -> float | None:
        ...
        try:
            from backend.core.schedule_core import Scheduler
    ...
    def _sync_work_to_state(self, job_name: str, hourly_rate: float,
        ...
        """
        try:
            from backend.core.model import Job as FREJob
    ...
    def _import_schedule(self):
        try:
            from backend.core.date_parser import parse_schedule as _parse_dates
        except ImportError:
            tk.Label(self._body,
        ...
        def _do_import():
            ...
            from backend.core.date_parser import parse_schedule as _parse_dates
            from backend.core.schedule_event import ScheduleEvent as _SE
            ...
            # ── Show saved summary ────────────────────────────────────────────
            if saved_shifts:
                from backend.core.schedule_core import _fmt12
